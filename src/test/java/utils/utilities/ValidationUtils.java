...
public class ValidationUtils
{
    ...
    public static Logger Log = LogManager.getLogger(utils.logs.Log.class);
...
//				win.add(winHandle);
    ...
    public static synchronized boolean VerifyPageNavigation(String PageName, String PagePattern)
    {
        try
        {
            String pattern = "https:\\/\\/" +  BaseTest.environment + "\\." + BaseTest.client + "\\.com\\/management\\/[a-zA-Z0-9]\\w+/" + PagePattern ;
            String Url = BaseTest.getDriver().getCurrentUrl();
            Matcher matcher = Pattern.compile(pattern,Pattern.MULTILINE).matcher(Url);

            if (matcher.find())
            {
                ExtentTestManager.createAssertTestStepWithScreenshot("Verify URL contains <b>" + PagePattern + "</b> after navigating to " + WordUtils.capitalize(PageName),
                        Status.PASS, "Url contains " + PagePattern + " as : " + Url, false);
                return true;
            }
            else
                ExtentTestManager.createAssertTestStepWithScreenshot("Verify URL contains <b>" + PagePattern + "</b> after navigating to " + WordUtils.capitalize(PageName),
                        Status.WARNING,"Url contains " + PagePattern + " as : " + Url, false);
        }
        catch(Exception e)
        {
            e.printStackTrace();
